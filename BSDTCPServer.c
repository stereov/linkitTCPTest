...
It opens the WiFi bearer by vm_bearer_open(). Once the bearer is opened, it will create a sub thread by vm_thread_create(). In the sub thread, it will bind itself to port 40000 by bind() and start listening to requests. When a request is received, it will 
receive the data by recv() and send the response by send();
...
#define AP_SSID "stereovision"    //填入自己的无线SSID
#define AP_PASSWORD "lihui1981"   //密码
#define AP_AUTH_MODE  VM_WLAN_AUTHORIZE_MODE_WPA2_ONLY_PSK
#define RESPONSE_STRING "Hello Client"   //返回的文字
